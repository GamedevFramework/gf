/*
 * Gamedev Framework (gf)
 * Copyright (C) 2016-2019 Julien Bernard
 *
 * This software is provided 'as-is', without any express or implied
 * warranty.  In no event will the authors be held liable for any damages
 * arising from the use of this software.
 *
 * Permission is granted to anyone to use this software for any purpose,
 * including commercial applications, and to alter it and redistribute it
 * freely, subject to the following restrictions:
 *
 * 1. The origin of this software must not be misrepresented; you must not
 *    claim that you wrote the original software. If you use this software
 *    in a product, an acknowledgment in the product documentation would be
 *    appreciated but is not required.
 * 2. Altered source versions must be plainly marked as such, and must not be
 *    misrepresented as being the original software.
 * 3. This notice may not be removed or altered from any source distribution.
 */
#include <gf/Transition.h>

#include <gf/Effect.h>
#include <gf/RenderTarget.h>
#include <gf/Texture.h>

namespace gf {
#ifndef DOXYGEN_SHOULD_SKIP_THIS
inline namespace v1 {
#endif

  Transition::Transition()
  : m_texture1(nullptr)
  , m_texture2(nullptr)
  , m_textureRect(RectF::fromPositionSize({ 0.0f, 0.0f }, { 1.0f, 1.0f }))
  , m_effect(nullptr)
  , m_totalTime(0.0f)
  , m_time(0.0f)
  {
    updateTexCoords();
  }

  void Transition::start(float time)
  {
    m_totalTime = time;
    m_time = 0.0f;
  }

  void Transition::stop(void)
  {
    m_time = 0.0f;
    m_totalTime = 0.0f;
  }

  void Transition::update(float time)
  {
    m_time += time;

    if (m_effect)
    {
      m_effect->setProgress(m_time / m_totalTime);
    }
  }

  void Transition::setTextures(const Texture& texture1, const Texture& texture2) {
    m_texture1 = &texture1;
    m_texture2 = &texture2;
    updatePositions();
  }

  void Transition::draw(RenderTarget& target, const RenderStates& states) {
    if (m_texture1 == nullptr || m_texture2 == nullptr) {
      return;
    }

    RenderStates localStates = states;

    localStates.shader = m_effect;
    localStates.texture[0] = m_texture1;
    localStates.texture[1] = m_texture2;
    target.draw(m_vertices, 4, PrimitiveType::TriangleStrip, localStates);
  }

  void Transition::updatePositions() {
    if (m_texture1 == nullptr || m_texture2 == nullptr) {
      return;
    }

    auto texSize = m_texture1->getSize();
    auto size = m_textureRect.getSize();
    float width = texSize.width * size.width;
    float height = texSize.height * size.height;

    m_vertices[0].position = {  0.0f,   0.0f };
    m_vertices[1].position = {  0.0f, height };
    m_vertices[2].position = { width,   0.0f };
    m_vertices[3].position = { width, height };
  }

  void Transition::updateTexCoords() {
    /* use the usual texture coordinates (0,0) in the bottom left as the
     * texture has been generated by a RenderTexture.
     */
    m_vertices[0].texCoords = { m_textureRect.min.x, m_textureRect.max.y };
    m_vertices[1].texCoords = { m_textureRect.min.x, m_textureRect.min.y };
    m_vertices[2].texCoords = { m_textureRect.max.x, m_textureRect.max.y };
    m_vertices[3].texCoords = { m_textureRect.max.x, m_textureRect.min.y };
  }

#ifndef DOXYGEN_SHOULD_SKIP_THIS
}
#endif
}
