...
inline namespace v2 {
  ...
  Grid Grid::createOrthogonal(Vector2i gridSize, Vector2f cellSize) {
    return Grid(gridSize, std::make_unique<OrthogonalCells>(gridSize, cellSize));
  }

  Grid Grid::createIsometric(Vector2i gridSize, Vector2f cellSize) {
    return Grid(gridSize, std::make_unique<IsometricCells>(gridSize, cellSize));
  }

  Grid Grid::createStaggered(Vector2i gridSize, Vector2f cellSize, CellAxis axis, CellIndex index) {
    return Grid(gridSize, std::make_unique<StaggeredCells>(gridSize, cellSize, axis, index));
  }

  Grid Grid::createHexagonal(Vector2i gridSize, Vector2f cellSize, float sideLength, CellAxis axis, CellIndex index) {
    return Grid(gridSize, std::make_unique<HexagonalCells>(gridSize, cellSize, sideLength, axis, index));
  }

  Grid Grid::createHexagonal(Vector2i gridSize, float radius, CellAxis axis, CellIndex index) {
    return Grid(gridSize, std::make_unique<HexagonalCells>(gridSize, radius, axis, index));
  }
  ...
  RectF Grid::getLocalBounds() const {
    ...
    return m_properties->computeBounds();
  }
