...
inline namespace v1 {
  ...
  ConsoleFont::ConsoleFont(const Image& image, ConsoleFontFormat format, Vector2i size)
  : m_mapping(MappingSize, 0x00)
  , m_format(format)
  , m_size(0, 0)
  , m_characterSize(0, 0)
  {
    ...
    if (size != gf::vec(0, 0)) {
      ...
      switch (format.mapping) {
        ...
        case ConsoleFontFormat::Custom:
          ...
          throw std::runtime_error("Undefined size for a custom font");
      }
    }
    ...
    if (imageSize.width % m_size.width != 0 || imageSize.height % m_size.height != 0) {
      ...
      throw std::runtime_error("Image size is not a multiple of font layout");
    }
  ...
  BitmapConsoleFont::BitmapConsoleFont(const Image& image, ConsoleFontFormat format, Vector2i size)
  : ConsoleFont(image, format, size)
  {
  ...
  BitmapConsoleFont::BitmapConsoleFont(const Path& filename, ConsoleFontFormat format, Vector2i size)
  : BitmapConsoleFont(Image(filename), format, size)
  {
    logFormat(filename);
  }
  ...
  ColoredConsoleFont::ColoredConsoleFont(const Image& image, ConsoleFontFormat format, Vector2i size)
  : ConsoleFont(image, format, size)
  {
    Vector2i imageSize = image.getSize();
    Image copy(image);
    ...
    switch (format.transparency) {
      ...
      case ConsoleFontFormat::Grayscale:
        ...
        throw std::runtime_error("Colored console font can not have grayscale transparency.");
  ...
  ColoredConsoleFont::ColoredConsoleFont(const Path& filename, ConsoleFontFormat format, Vector2i size)
  : ColoredConsoleFont(Image(filename), format, size)
  {
    logFormat(filename);
  }
